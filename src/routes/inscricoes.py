...
def criar_inscricao(insc: InscricaoSchema):
    ...
    # Validação manual das Chaves Estrangeiras (Segurança do SQLite)
...
# 2. LISTAR INSCRIÇÕES COM JOIN (GET)
@router.get("/")
def listar_inscricoes():
    try:
        ...
        cursor = conexao.cursor()
        query = """
            SELECT i.id_inscricao, p.nome AS nome_participante, c.modalidade AS nome_campeonato, i.status_inscricao
            FROM inscricoes i
        ...
        return [
            {"id_inscricao": r[0], "participante": r[1], "campeonato": r[2], "status": r[3]}
            for r in resultados
        ]
    except Exception as e:
...
# 3. ATUALIZAR STATUS DA INSCRIÇÃO (PUT)
...
def atualizar_status_inscricao(id_inscricao: int, insc: InscricaoSchema):
    try:
        ...
        conexao.commit()
        return {"mensagem": "Status da inscrição atualizado com sucesso!"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
    finally:
        conexao.close()

# 4. CANCELAR/DELETAR INSCRIÇÃO (DELETE)
